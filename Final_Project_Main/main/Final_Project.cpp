...
const char *textureFiles[] = {
    "../main/textures/sky1.png",
    "../main/textures/fasade_sky2.png",
    "../main/textures/fasade_sky1.png",
    "../main/textures/fasade_sky1.png",
    "../main/textures/fasade_sky4.png",
    "../main/textures/fasade_sky5.png",
    "../main/textures/fasade_sky6.png",
...
struct Museum {
    ...
    void initialize(glm::vec3 position, glm::vec3 scale, const char *textureFile) {
        ...
        char *selectedTexture = "../main/textures/gpo56.png";
        textureID = LoadTextureTileBox(selectedTexture);
...
struct Bars {
    ...
    void initialize(glm::vec3 position, glm::vec3 scale, const char *textureFile) {
        ...
        char *selectedTexture = "../main/textures/bar1.png";
        textureID = LoadTextureTileBox(selectedTexture);
...
struct Roof {
    ...
    void initialize(glm::vec3 position, glm::vec3 scale, const char *textureFile) {
        ...
        char *selectedTexture = "../main/textures/grey44.png";
        textureID = LoadTextureTileBox(selectedTexture);
...
struct Pillar {
    ...
    void initialize(glm::vec3 position, glm::vec3 scale) {
        ...
        textureID = LoadTextureTileBox("../main/textures/grey44.png");
...
struct Plateau {
    ...
    void initialize(glm::vec3 position, glm::vec3 scale) {
        ...
        textureID = LoadTextureTileBox("../main/textures/grass2.png");
...
struct Street {
    ...
    void initialize(glm::vec3 position, glm::vec3 scale) {
        ...
        textureID = LoadTextureTileBox("../main/textures/street3.png");
...
struct Spire {
    ...
    void initialize(glm::vec3 position, glm::vec3 scale) {
        ...
        if (Status != GL_FRAMEBUFFER_COMPLETE) {
            std::cerr << "Failed to FBO." << std::endl;
        }
        glBindFramebuffer(GL_FRAMEBUFFER, 0);

        // Create and compile our GLSL program from the shaders
        programLightID = LoadShadersFromFile("../main/box.vert", "../main/box.frag");
        if (programLightID == 0) {
            std::cerr << "Failed to load shaders." << std::endl;
        }
        ...
        //textureID = LoadTextureTileBox("../main/textures/grey.png");
    ...
    void shadowRender(glm::mat4 lightPositionMatrix) {
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, depthMap);
        ...
        glm::mat4 mvp = lightPositionMatrix;
        glUniformMatrix4fv(mvpMatrixID, 1, GL_FALSE, &mvp[0][0]);
    ...
    // void render1(glm::mat4 cameraMatrix, float exposureVal) {
    //     glActiveTexture(GL_TEXTURE0);
    //     glBindTexture(GL_TEXTURE_2D, depthMap);
    //
    //     glUseProgram(programID);
    ...
    void render(glm::mat4 cameraMatrix, float exposureVal, glm::mat4 lightPositionMatrix) {
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, depthMap);
        ...
        glUniformMatrix4fv(lightSpaceMatrixID, 1, GL_FALSE, &mvp1[0][0]);
...
struct MyBot {
    ...
    void computeGlobalNodeTransform(const tinygltf::Model &model,
        ...
        const tinygltf::Node &node = model.nodes[nodeIndex];
        for (int childIndex: node.children) {
            computeGlobalNodeTransform(model, localTransforms, childIndex, globalTransforms[nodeIndex],
                                       globalTransforms);
        }
    }
    ...
    std::vector<SkinObject> prepareSkinning(const tinygltf::Model &model) {
        ...
        for (size_t i = 0; i < model.skins.size(); i++) {
            ...
            computeGlobalNodeTransform(model, localNodeTransforms, rootNodeIndex, parentTransform,
                                       skinObject.globalJointTransforms);
            //last parameter automatically saves global joint transforms

            for (size_t j = 0; j < skin.joints.size(); j++) {
                int jointNodeIndex = skin.joints[j];

                std::cout << "JointNodeIndex: " << jointNodeIndex << std::endl;
                skinObject.jointMatrices[j] = skinObject.globalJointTransforms[jointNodeIndex] * skinObject.
    ...
    std::vector<AnimationObject> prepareAnimation(const tinygltf::Model &model) {
        ...
        for (const auto &anim: model.animations) {
            ...
            for (const auto &sampler: anim.samplers) {
                ...
                const unsigned char *inputPtr = &inputBuffer.data[
                    inputBufferView.byteOffset + inputAccessor.byteOffset];
                ...
                const unsigned char *outputPtr = &outputBuffer.data[
                    outputBufferView.byteOffset + outputAccessor.byteOffset];
    ...
    void initialize() {
        ...
        if (!loadModel(model, "../main/model/bot/bot.gltf")) {
            return;
        }
        ...
        // Create and compile our GLSL program from the shaders
        programID = LoadShadersFromFile("../main/shader/bot.vert", "../main/shader/bot.frag");
        if (programID == 0) {
            std::cerr << "Failed to load shaders." << std::endl;
        }
...
int main(void) {
    ...
    glm::mat4 light_projectionMatrix = glm::perspective(glm::radians(depthFoV),
                                                        (float) shadowMapWidth / shadowMapHeight, depthNear, depthFar);

    glm::vec3 light_lookAt = lightPosition + lightLookat;
    ...
    Building building2;
    building2.initialize(glm::vec3(200, 10, 200), glm::vec3(50, 500, 70), 1);
    Building building3;
    building3.initialize(glm::vec3(200, 10, 400), glm::vec3(50, 500, 70), 5);
    Building building4;
    building4.initialize(glm::vec3(200, 10, -180), glm::vec3(50, 700, 70), 6);
    Building building5;
    building5.initialize(glm::vec3(200, 10, -380), glm::vec3(50, 600, 70), 1);
    Building building6;
    building6.initialize(glm::vec3(-200, 430, -100), glm::vec3(90.0f, 300.0f, 90.0f), 1);
    Bars bar1;
    bar1.initialize(glm::vec3(-200, 10, -100), glm::vec3(90, 120, 90), "../main/textures/bar1.png");

    Museum museum;
    museum.initialize(glm::vec3(-270, 10, 260), glm::vec3(100, 210, 190), "../main/textures/gpo2.png");

    Pillar pillar1;
    pillar1.initialize(glm::vec3(-120, 10, 170), glm::vec3(10, 40, 10));

    Pillar pillar2;
    pillar2.initialize(glm::vec3(-120, 10, 230), glm::vec3(10, 40, 10));

    Pillar pillar3;
    pillar3.initialize(glm::vec3(-120, 10, 290), glm::vec3(10, 40, 10));
    ...
    Roof roof;
    roof.initialize(glm::vec3(-150, 230, 260), glm::vec3(50, 30, 110), "../main/textures/roof.png");
    ...
    do {
        ...
        lightSpaceView = glm::lookAt(lightPosition, light_lookAt, lightUp);
        glm::mat4 light_vp = light_projectionMatrix * lightSpaceView;
        glBindFramebuffer(GL_FRAMEBUFFER, depthMapFBO);
        glClear(GL_DEPTH_BUFFER_BIT);

        //spire.shadowRender(light_vp);
        if (saveDepth) {
...
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
    ...
    const float z_min = -10000.0f, z_max = 10000.0f;
    const float y_min = 20.0f, y_max = 500.0f;

    // Calculate the new position based on input
    glm::vec3 newPosition = cameraPosition;

    if (key == GLFW_KEY_UP && (action == GLFW_PRESS || action == GLFW_REPEAT))
        newPosition += cameraFront * velocity; // Move forward
    if (key == GLFW_KEY_DOWN && (action == GLFW_PRESS || action == GLFW_REPEAT))
        newPosition -= cameraFront * velocity; // Move backward
    if (key == GLFW_KEY_LEFT && (action == GLFW_PRESS || action == GLFW_REPEAT))
        newPosition -= cameraRight * velocity; // Move left
    if (key == GLFW_KEY_RIGHT && (action == GLFW_PRESS || action == GLFW_REPEAT))
        newPosition += cameraRight * velocity; // Move right
    if (key == GLFW_KEY_P && (action == GLFW_PRESS || action == GLFW_REPEAT))
        newPosition += cameraUp * velocity; // Move up
    if (key == GLFW_KEY_L && (action == GLFW_PRESS || action == GLFW_REPEAT))
        newPosition -= cameraUp * velocity; // Move down

    // Apply boundaries by clamping the new position
    if (newPosition.x < x_min) newPosition.x = x_min;
    if (newPosition.x > x_max) newPosition.x = x_max;
    if (newPosition.y < y_min) newPosition.y = y_min;
    if (newPosition.y > y_max) newPosition.y = y_max;
    if (newPosition.z < z_min) newPosition.z = z_min;
    if (newPosition.z > z_max) newPosition.z = z_max;
