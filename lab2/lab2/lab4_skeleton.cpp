    ...
    std::vector<glm::mat4> &nodeTransforms)
{
    for (const auto &channel : anim.channels) {
        ...
        const float *outputBuf = reinterpret_cast<const float*>(&outputBuffer.data[outputBufferView.byteOffset + outputAccessor.byteOffset]);

        if (channel.target_path == "translation") {
            // Interpolate joint translations
    ...
    // Add a global translation for the root node to move forward in the X direction
    if (!nodeTransforms.empty()) {
        nodeTransforms[0] = glm::translate(nodeTransforms[0], glm::vec3(time * 10.0f, 0.0f, 0.0f)); // Move forward along X
    }
}
	...
	void update(float time) {
		...
		if (model.animations.size() > 0) {
			...
			std::vector<glm::mat4> nodeTransforms(skin.joints.size(), glm::mat4(1.0));

			updateAnimation(model, animation, animationObject, time, nodeTransforms);
			// TO:
			// Compute global transforms
			computeGlobalNodeTransform(model, nodeTransforms, skin.joints[0], glm::mat4(1.0), globalTransforms);
		}
	}
