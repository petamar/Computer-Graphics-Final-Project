...
static GLuint LoadTextureTileBox(const char *texture_file_path) {
	...
	//Set so the texture does not repeat
...
struct Skybox {
	...
	glm::vec3 scale;		// Size of the box
	...
	//reverse faces as viewpoint is inside of the cube
	...
	//set UV coordinates, but I have no idea why these numbers
	//after a lot of trying I found out 0.2 is max no y axes, so I changed it according to that
	...
	void initialize(glm::vec3 position, glm::vec3 scale) {
		...
		this->scale = glm::vec3(100.0f,100.0f,100.0f);
	...
	void render(glm::mat4 cameraMatrix) {
		...
		// Model transform
		glm::mat4 modelMatrix = glm::translate(glm::mat4(1.0f), position);  // Position the cube at the origin
		modelMatrix = glm::scale(modelMatrix, scale);                       // Scale the cube appropriately
...
int main(void)
{
	...
	Skybox s;
	s.initialize(glm::vec3(0, 0, 0), glm::vec3(10, 10, 10));
